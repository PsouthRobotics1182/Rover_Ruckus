...
public class FineSlide {
    ...
    private ModernRoboticsI2cRangeSensor range2;
    ...
    public FineSlide(LinearOpMode opMode, DcMotor.RunMode mode) {
        ...
        range2 = opMode.hardwareMap.get(ModernRoboticsI2cRangeSensor.class, "range2");
    ...
    public boolean strafingRangeOtherSide(int mm, double power) {
        power = -power;
        double error = mm - range2.getDistance(DistanceUnit.MM);
        if (Math.abs(error) > ANGLE_TOLERENCE && opMode.opModeIsActive()) {
            //power = power * error/Math.abs(mm);
            //double correction = imu.correction(0);
            //setLeftPower(correction);
    ...
    public void strafeRangeOtherSide(int mm, double power) {
        resetEncoders();
        while (strafingRangeOtherSide(mm, power) && opMode.opModeIsActive()) {
            opMode.telemetry.addData("Driving", "Now");
            opMode.telemetry.update();
        }
        stop();
